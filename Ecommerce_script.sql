...
update ecommerce.customers_new set DOB = str_to_date(DOB,"%d/%m/%Y");
...
#(11 Find the average and total revenue by each subcategory for the  categories which are among top 5 in terms of quantity sold.
select prod_subcat, avg(total_revenue)as avg_revenue, sum(total_revenue) as total_revenue from
(select prod_subcat,prod_cat,sum(case when total_amt > 0 then total_amt else 0 end) as total_revenue,
sum(case when Qty >0 then Qty else 0 end) as total_sales_qty from merged_ecommerce
 group by prod_cat,prod_subcat order by total_sales_qty desc limit 5) as age group by prod_subcat;

#(12 which store type sells the maximum products; by value of sales ammount and by quentity.
